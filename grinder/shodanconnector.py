...
from grinder.errors import ShodanGathererInitError, ShodanGathererSearchError


class ShodanConnector:
    @exception_handler(expected_exception=ShodanGathererInitError)
    def __init__(self, api_key=DefaultValues.API_KEY):
        self.api = Shodan(api_key)
        self.results: list = []
        self.shodan_results_count: int = 0
        self.real_results_count: int = 0

    @exception_handler(expected_exception=ShodanGathererSearchError)
